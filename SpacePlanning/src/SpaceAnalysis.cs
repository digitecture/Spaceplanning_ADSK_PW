...
namespace SpacePlanning
{
    ...
    public static class SpaceAnalysis
    {
            ...
            int colorScheme = 0, bool colorProgramSeparate = false, int opacity = 10)
        {
            ...
            for (int i = 0; i < deptDataInp.Count; i++)
            {
                List<ProgramData> progsInDept = deptDataInp[i].ProgramsInDept;
        ...
        //Provides information related to program data
        [MultiReturn(new[] { "DisplayGeomList" })]
        internal static Dictionary<string, object> VisualizeDeptPrograms2D3D(List<DeptData> deptDataInp, double height = 0, int transparency = 255, int colorScheme = 0, bool colorProgramSeparate = false)
        {
            List<DeptData> deptData = deptDataInp;
            deptDataInp = deptData.Select(x => new DeptData(x)).ToList(); // example of deep copy
            ...
            double heightPolylines = 0, bool fullProgramNames = true)
        {
            ...
            for (int i = 0; i < deptData.Count; i++)
            {
                int indexHeight = deptData[i].DeptFloorLevel;
                heightPlan = deptData[i].FloorHeightList[indexHeight] + height;
                heightPolylines = heightPlan - 0.2;
                List<ProgramData> progInDept = deptData[i].ProgramsInDept;
                List<List<Polygon>> polyDeptList = new List<List<Polygon>>();
                List<List<Point>> ptDeptList = new List<List<Point>>();
                List<string> nameDeptList = new List<string>();
        ...
        /// </search>
        [MultiReturn(new[] { "progPolygons", "progPolyOrigin", "progNameAsText" })]
        internal static List<Dictionary<string, object>> VisualizeProgramPolyLinesAndOrigin3D(List<DeptData> deptDataInp, double height = 0, double heightPolylines = 0, bool fullProgramNames = true)
        {
            List<DeptData> deptData = deptDataInp;
            ...
            for (int i = 0; i < floorHeightList.Count; i++)
            {
                ...
                Dictionary<string, object> displayObj = VisualizeProgramPolyLinesAndOrigin2D(deptDataInp, floorHeightList[i], floorHeightList[i]-0.1, fullProgramNames);
                if (displayObj != null) visObjList.Add(displayObj);
            }
            return visObjList;
        }
        ...
        /// </search>
        [MultiReturn(new[] { "progPolygons", "progPolyOrigin", "progNameAsText" })]
        internal static List<Dictionary<string, object>> VisualizeProgramPolyLinesAndOrigin2D3D(List<DeptData> deptDataInp, double height =0, double heightPolylines = 0, bool fullProgramNames = true)
        {
            List<DeptData> deptData = deptDataInp;
            Dictionary<string, object> obj = new Dictionary<string, object>();
            deptDataInp = deptData.Select(x => new DeptData(x)).ToList(); // example of deep copy
            if (deptDataInp[0].Mode3D) { return VisualizeProgramPolyLinesAndOrigin3D(deptDataInp, height, heightPolylines, fullProgramNames); }
            else { obj = VisualizeProgramPolyLinesAndOrigin2D(deptDataInp, height, heightPolylines, fullProgramNames); }
